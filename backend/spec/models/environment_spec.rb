...
RSpec.describe Environment, type: :model do
  ...
  it 'is invalid with unsupported status' do
    env = described_class.new(valid_attributes.merge(status: 'unknown'))
    expect(env).not_to be_valid
  end

  describe '#expired?' do
    it 'returns true when expires_at is in the past' do
      env = described_class.new(valid_attributes.merge(expires_at: 1.minute.ago))
      expect(env.expired?).to be true
    end

    it 'returns false when expires_at is in the future' do
      env = described_class.new(valid_attributes)
